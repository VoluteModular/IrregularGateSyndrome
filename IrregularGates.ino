...
const int dipPins[] = {9,10,11,12,13}; // Pins for jumpers to set options
const int potPins[] = {0,1,2}; // Pins connected to the randomness pots.
bool dipState[] = {false, false, false, false, false}; //Set the starting state for the config jumpers
unsigned long pulseLength = 8; //Default pulse length
...
void checkDipPins() {   // This checks the status of the jumpers to set the variables
  ...
  if (dipState[0] == true) pulseLength = 200; // pin one sets the pulse length
  else pulseLength = 4;     
